defmodule TrinityCoordinator.Training.SepCMAESRecombinationTest do
  ...
  test "uniform recombination preserves arithmetic mean" do
    ...
    assert Nx.all_close(mean, Nx.tensor([10.0 / 3.0, 10.0 / 3.0]), atol: 1.0e-6)
  end
  ...
  test "rank-weighted mean favors higher ranked vectors" do
    ...
    refute Nx.all_close(uniform, weighted)
    [weighted_x, _weighted_y] = Nx.to_flat_list(weighted)
    [uniform_x, _uniform_y] = Nx.to_flat_list(uniform)
